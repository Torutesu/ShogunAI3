...
## 4. `mirror_search_blobs` should run CPU-bound work on `spawn_blocking` (final review IMP-1) — RESOLVED in Phase 2.1.4.1 (PR pending)
...
**Resolution**: Phase 2.1.4.1 wraps the entire `search_cloud_blobs` call in `tokio::task::spawn_blocking` and bridges the inner async network calls through the dedicated `MIRROR_RUNTIME` (matching `mirror_sync_now`'s pattern). `mirror::sync::mirror_runtime` is now `pub(crate)` so the IPC layer can use it. Existing `mirror_search_blobs_locked_returns_locked` defensive-check test still passes; full mirror::search and mirror::http suites green.

## 5. Merge-layer automated coverage gap (final review nit 7) — RESOLVED in Phase 2.1.4.1 (PR pending)
...
**Resolution**: Phase 2.1.4.1 adds `tests/e2e/memory-search-merge.spec.js` with 5 Playwright tests driving `window.SHOGUN_RUNTIME.executeAction('memory.search', ...)` end-to-end through `runMemorySearchMerged`. Mocks `mirror_status`, `shogun_memory_search`, and `mirror_search_blobs` via the `__shogunMockOverrides` seam. Covers: cloud-disabled, cloud-locked, dedupe-newer-wins + similarity rank, kinds-filter post-application to cloud hits, and 5s cloud-timeout fallback.
