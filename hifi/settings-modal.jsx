...
// ---------------------------------------------------------------------------
// Phase 2.1.4 T5 — Cloud Mirror pane
...
// in T3 (`hifi/lib/action-registry.js`).
// ---------------------------------------------------------------------------
 ...
 */
function ConfirmTypedText({ word, title, description, confirmLabel, onConfirm, onCancel }) {
...
function MirrorOnboardingModal({ onClose, onComplete, run, toast }) {
  const [step, setStep] = useStateS(1); // 1: URL, 2: Code, 3: Name, 4: Pass
  const [serverUrl, setServerUrl] = useStateS('');
  const [regCode, setRegCode] = useStateS('');
  const [deviceName, setDeviceName] = useStateS('');
...
function MirrorDeviceRow({ device, run, toast, refreshDevices }) {
  const [renaming, setRenaming] = useStateS(false);
  const [newName, setNewName] = useStateS(device.device_name || '');
  const [showDelete, setShowDelete] = useStateS(false);
  const [busy, setBusy] = useStateS(false);
...
function MirrorDisabledView({ refreshStatus, run, toast }) {
  const [showOnboarding, setShowOnboarding] = useStateS(false);
  return (
...
function MirrorLockedView({ refreshStatus, run, toast }) {
  const [pass, setPass] = useStateS('');
  const [busy, setBusy] = useStateS(false);
...
function MirrorActiveView({ status, refreshStatus, run, toast }) {
  const { setPane } = React.useContext(SettingsHydrationContext);
  const [devices, setDevices] = useStateS([]);
  const [devicesLoading, setDevicesLoading] = useStateS(false);
  const [devicesTruncated, setDevicesTruncated] = useStateS(false);
  const [showDisable, setShowDisable] = useStateS(false);
  const [syncing, setSyncing] = useStateS(false);
  const [resetting, setResetting] = useStateS(false);
...
function PaneCloudMirror() {
  const { run, toast } = useRuntimeActions();
  const [status, setStatus] = useStateS(null);
  const [statusLoading, setStatusLoading] = useStateS(false);
