...
- **`shogun_brief_get`** / **`brief.get`**: v1 path calls `llm::brief_generate` (OpenAI-compatible). When the v2 gate fires (payload `forceV2` / `version: "2" | "2.0"`, or `sections.brief.morningBriefVersion === "2"`), Rust builds `MorningBriefCandidate` objects from local memory (`google_calendar` + `gmail`, ≤ 3-day window, cap 8 + 6) and `meeting_store::list_meetings` (cap 6), enriches each with up to 3 `related_kioku_hits` from a local FTS5 search (synthetic relevance 0.95 / 0.80 / 0.65, self-id excluded), caps the total at 20, and pipes the JSON array into the Node **`amc-pipeline`** (`node hifi/amc-pipeline/src/cli.js --stdin`, 60s timeout). The v1 response is mapped to v2 (`hifi/schemas/morning-brief-v2.schema.json`) via `brief_v2_adapter`. When no candidates exist locally, the pipeline is invoked on its bundled fixture with `--dry` instead. On sidecar / adapter failure or a `{skipped: true}` response the endpoint falls back to `morning_brief_v2_stub` with a `fallbackReason` string and the error is recorded in `diagnostics::recentErrors`. Responses include `sourceMode` (`"stdin_candidates"` or `"fixture_dry"`) and `candidateCount`. The Anthropic API key for live LLM composition is stored in the Keychain via `app_anthropic_api_key_set/status/clear` and injected into the sidecar's `ANTHROPIC_API_KEY` only when the user hasn't already exported it; the pipeline itself decides `--dry` vs live based on whether the env var is set.
