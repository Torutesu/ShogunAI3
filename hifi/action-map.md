...
- **`shogun_brief_get`** / **`brief.get`**: v1 path calls `llm::brief_generate` (OpenAI-compatible). When the v2 gate fires (payload `forceV2` / `version: "2" | "2.0"`, or `sections.brief.morningBriefVersion === "2"`), Rust spawns the Node **`amc-pipeline`** (`node hifi/amc-pipeline/src/cli.js --dry`, 60s timeout) as a one-shot subprocess, parses v1 JSON, and maps it to the v2 schema (`hifi/schemas/morning-brief-v2.schema.json`) via `brief_v2_adapter`. On sidecar failure the response falls back to `morning_brief_v2_stub` with a `fallbackReason` string and the error is recorded in `diagnostics::recentErrors`. Phase B.1 uses the bundled fixture; Phase B.2 will supply real candidates + live LLM.
