...
- `decision_graph_hits` and `weekly_brief.stuck_days` on candidates are intentionally left empty / unset. There is no decision-graph store in this build, and `created_at` on memory rows is the **ingest** timestamp (not the upstream "received" or "last touched" time), so deriving stuck-days from row age would mislead the composer. Both will be wired once an upstream signal (Gmail thread last-reply timestamp, decision graph storage, etc.) lands.
