...
fn ensure_shape(mut v: Value) -> Value {
    ...
    if let Some(sections) = v.get_mut("sections").and_then(|s| s.as_object_mut()) {
        ...
        if let Some(o) = mem.as_object_mut() {
            ...
            o.entry("autoDigestIntervalMins".to_string())
                .or_insert(json!(360));
            o.entry("autoDigestLang".to_string()).or_insert(json!("en"));
            o.entry("semanticRerank".to_string())
                .or_insert(json!(false));
        }
        let cap = sections
        ...
        if let Some(o) = cap.as_object_mut() {
            ...
            o.entry("sampleIntervalSecs".to_string())
                .or_insert(json!(4));
        ...
        if let Some(o) = billing.as_object_mut() {
            ...
            o.entry("currentPeriodEnd".to_string())
                .or_insert(json!(null));
        ...
        if let Some(o) = kioku_graph.as_object_mut() {
            o.entry("capture_to_mem_captures".to_string())
                .or_insert(json!(true));
            o.entry("worker_enabled".to_string()).or_insert(json!(true));
            o.entry("read_path".to_string()).or_insert(json!("graph"));
            o.entry("poll_interval_secs".to_string())
                .or_insert(json!(30));
            o.entry("max_jobs_per_tick".to_string()).or_insert(json!(5));
        ...
        if let Some(o) = mtg.as_object_mut() {
            o.entry("appDetectAlerts".to_string())
                .or_insert(json!(true));
            o.entry("remindersOn".to_string()).or_insert(json!(true));
            o.entry("reminderMins".to_string()).or_insert(json!("5"));
            o.entry("excludeNoGuests".to_string())
                .or_insert(json!(true));
            o.entry("notifScope".to_string())
                .or_insert(json!("confirmed_only"));
...
fn save_patch_inner(payload: &Value) -> Result<Value, AppError> {
    ...
    let entry_obj = entry
        .as_object_mut()
        .ok_or_else(|| AppError::InvalidInput("section value must be an object".to_string()))?;
...
mod tests {
    ...
    fn migrate_kioku_flags_backfills_legacy_doc() {
        ...
        assert_eq!(
            migrated
                .pointer("/sections/meetings/appDetectAlerts")
                .and_then(|v| v.as_bool()),
            Some(true)
        );
        assert_eq!(
    ...
    fn load_missing_file_returns_shaped_defaults() {
        ...
        assert_eq!(
            doc.pointer("/sections/capture/retentionDays")
                .and_then(|v| v.as_u64()),
            Some(30)
        );
        assert_eq!(
        ...
        assert_eq!(
            doc.pointer("/sections/kioku_cost/monthly_cap_usd")
                .and_then(|v| v.as_f64()),
            Some(10.0)
        );
        assert_eq!(
            doc.pointer("/sections/llm/extractionModel")
                .and_then(|v| v.as_str()),
    ...
    fn load_migrates_legacy_file_and_persists_bump() {
        ...
        assert_eq!(
            doc.pointer("/sections/meetings/autoStartOnCalendar")
                .and_then(|v| v.as_bool()),
            Some(true)
        );
    ...
    fn save_patch_merges_into_section_and_preserves_others() {
        ...
        let doc =
            save_patch(&json!({ "section": "memory", "autoDigest": true, "autoDigestLang": "jp" }))
                .expect("second save");

        assert_eq!(
            doc.pointer("/sections/memory/autoDigest")
                .and_then(|v| v.as_bool()),
            Some(true)
        );
        assert_eq!(
        ...
        assert_eq!(
            doc.pointer("/sections/capture/paused")
                .and_then(|v| v.as_bool()),
            Some(true)
        );
    ...
    fn save_patch_defaults_to_misc_section() {
        ...
        assert_eq!(
            doc.pointer("/sections/misc/someFlag")
                .and_then(|v| v.as_u64()),
            Some(42)
        );
    }
    ...
    fn upsert_integration_provider_creates_and_merges() {
        ...
        let doc = upsert_integration_provider("gmail", &json!({ "scope": "read" }))
