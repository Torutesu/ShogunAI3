...
async fn run_once(lang: &str) -> Result<(String, String), String> {
    ...
    let day_cache = summarizer_store::get_cached("day_rollup", &day_id, lang)
        .ok()
        .flatten();
    let day_should_run = window_has_summaries(day_ms, day_end, lang)
        && match &day_cache {
            ...
            Some(c) => summarizer_store::get_summaries_in_window(day_ms, day_end, lang)
    ...
    let week_cache = summarizer_store::get_cached("week_rollup", &week_id, lang)
        .ok()
        .flatten();
