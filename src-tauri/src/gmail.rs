...
pub async fn sync_inbox_to_memory(
  max_results: usize,
  days: Option<u32>,
) -> Result<Value, String> {
  ...
  let page_size = if days.is_some() { 500 } else { max_results.max(1) };
  ...
  for id in collected_ids.iter() {
    ...
    let (st, txt) = if first.0 == StatusCode::UNAUTHORIZED && google_oauth::credentials_can_refresh(&creds) {
      ...
      match gmail_get_message_metadata(&token, id).await {
        Ok(r) => r,
        Err(e) => {
          log::warn!("Skipping message {} after refresh (network error): {}", id, e);
          continue;
        }
      }
    } else {
