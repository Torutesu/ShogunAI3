...
pub fn parse_env(text: &str) -> Result<(String, String), OauthError> {
    ...
    for raw in text.lines() {
        ...
        let Some((key, val_raw)) = line.split_once('=') else {
            continue;
        };
        let key = key.trim();
...
pub fn load_oauth_credentials() -> Result<(String, String), OauthError> {
    ...
    let compile_sec = option_env!("SHOGUN_GOOGLE_CLIENT_SECRET")
        .unwrap_or("")
        .trim();
...
pub async fn exchange_code(
    ...
    let parsed: serde_json::Value = serde_json::from_str(&text)
        .map_err(|e| OauthError::Internal(format!("parse token JSON: {}", e)))?;
    ...
    let join = tokio::task::spawn_blocking(move || -> Result<(String, String), OauthError> {
      ...
      _ => (
        ...
        match outcome {
            ...
            CallbackOutcome::ProviderError(desc) => {
                Err(OauthError::Internal(format!("provider error: {}", desc)))
            }
