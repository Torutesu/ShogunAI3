...
pub fn inactivity_timeout_ms() -> u64 {
  settings_store::load()
    ...
    .and_then(|doc| {
      doc
        .pointer("/sections/meetings/inactivityMins")
        .and_then(|v| v.as_str())
...
pub async fn finalize_meeting(app: &AppHandle, meeting_id: &str, reason: &str) -> Result<Value, String> {
  ...
  let _ = app.emit("shogun-meetings-changed", json!({ "meeting_id": meeting_id }));
  Ok(out)
}
...
async fn check_inactivity(app: &AppHandle) {
  ...
  let is_video_meeting = session.is_video_provider_meeting(&meeting_id).unwrap_or(false);
