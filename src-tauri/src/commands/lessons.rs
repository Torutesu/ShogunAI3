#[tauri::command]
pub async fn shogun_lesson_capture_rejection(
    ...
    let rule = match crate::llm::anthropic_tool_complete(
        system,
        &user_content,
        &tool,
...
#[tauri::command]
pub async fn shogun_lesson_capture_tool_failure(
    payload: serde_json::Value,
) -> Result<serde_json::Value, String> {
    ...
    let attempted =
        serde_json::json!({"action": action, "payload": inner_payload, "agentId": agent_id});
    ...
    .await
    {
        Ok(input) => input
            ...
            .unwrap_or_else(|| {
                format!(
                    "{} failed with: {} — verify preconditions",
...
#[tauri::command]
pub async fn shogun_patterns_run_now(
...
#[tauri::command]
pub async fn shogun_supersession_run_now(
    _payload: serde_json::Value,
) -> Result<serde_json::Value, String> {
