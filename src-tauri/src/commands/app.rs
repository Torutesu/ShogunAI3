...
fn compute_sli_snapshot(now_ms: i64) -> Result<Value, String> {
    ...
    let rows = stmt
        .query_map(params![window_start_ms], |r| {
...
pub(crate) fn capture_status_snapshot(
    frontmost_focus: Option<crate::macos_frontmost::FrontmostFocus>,
) -> Value {
    let settings = settings_store::load().unwrap_or_else(|_| json!({}));
    let paused = settings
    ...
    json!({
      ...
      "frontmostFocus": frontmost_focus,
    })
}

fn screen_context_health_snapshot(
    frontmost_focus: Option<&crate::macos_frontmost::FrontmostFocus>,
    accessibility_trusted: Option<bool>,
    ax_snapshot: Option<&str>,
    ax_snapshot_source: Option<&str>,
    ax_diagnostic_reason: Option<&str>,
    ax_text_signal_present: Option<bool>,
    ax_text_chars: Option<u64>,
    ax_line_count: Option<u64>,
) -> Value {
    ...
    let window_title_source = frontmost_focus.and_then(|f| f.window_title_source.clone());
    let mut issues: Vec<&'static str> = Vec::new();
    let state = if accessibility_trusted == Some(false) {
        issues.push("Accessibility permission is not granted");
        "error"
    } else if frontmost_focus.is_none() {
        issues.push("frontmost app snapshot is unavailable");
        "error"
    } else {
        if window_title
            .as_deref()
        ...
        {
            issues.push("window title is unavailable");
        }
        if !ax_snapshot_present {
            issues.push(match ax_diagnostic_reason {
                Some("focused_element_unavailable") => "focused AX element is unavailable",
                Some("secure_text_field") => "focused control is a secure text field",
                Some("focused_element_fields_empty") => "focused AX element exposes no text fields",
                Some("unavailable_on_platform") => "AX snapshot is unavailable on this platform",
                _ => "AX snapshot is empty",
            });
    ...
    let label = match state {
        "ok" => "Screen context OK",
        "warn" => "Screen context partial",
        _ => "Screen context blocked",
    };
    let message = if issues.is_empty() {
        let ax_note = if ax_snapshot_source.is_some_and(|source| source.contains("tree")) {
            " AX tree fallback is active."
        } else {
            ""
        };
        let title_note = if window_title_source.as_deref() == Some("system_events") {
            " Window title is using System Events fallback."
        } else {
            ""
        };
        format!(
      "Frontmost app, bundle ID, window title, and AX context are being read.{ax_note}{title_note}"
    )
    } else {
        format!("{}.", issues.join("; "))
    };
    json!({
      ...
      "windowTitleSource": window_title_source,
      "axSnapshotSource": ax_snapshot_source,
      "axDiagnosticReason": ax_diagnostic_reason,
    })
}
...
pub fn shogun_dead_letter_list(payload: Value) -> Result<Value, String> {
    let limit = payload.get("limit").and_then(|x| x.as_i64()).unwrap_or(200);
    let source = payload
        .get("source")
        .and_then(|x| x.as_str())
        .filter(|s| !s.is_empty())
...
pub fn shogun_dead_letter_retry(payload: Value) -> Result<Value, String> {
    let limit = payload.get("limit").and_then(|x| x.as_i64()).unwrap_or(500);
    let source = payload
        .get("source")
        .and_then(|x| x.as_str())
        .filter(|s| !s.is_empty())
...
pub fn shogun_capture_status(payload: Value) -> Result<Value, String> {
    let mut status = capture_status_snapshot(crate::macos_frontmost::frontmost_focus_snapshot());
    if let Some(obj) = status.as_object_mut() {
        obj.insert("stub".to_string(), serde_json::Value::Bool(false));
        obj.insert("echo".to_string(), payload);
    }
    Ok(status)
}
...
#[tauri::command]
pub fn shogun_screen_context_probe(payload: Value) -> Result<Value, String> {
    let frontmost_focus = crate::macos_frontmost::frontmost_focus_snapshot();
    let capture_status = capture_status_snapshot(frontmost_focus.clone());
    let hummingbird_context =
        crate::hummingbird::capture_context_from_frontmost(frontmost_focus.clone());
    let screen_context_health = screen_context_health_snapshot(
        frontmost_focus.as_ref(),
        capture_status
            .pointer("/permissions/accessibilityTrusted")
            .and_then(|v| v.as_bool()),
        hummingbird_context
            .pointer("/axSnapshot")
            .and_then(|v| v.as_str()),
        hummingbird_context
            .pointer("/axSnapshotSource")
            .and_then(|v| v.as_str()),
        hummingbird_context
            .pointer("/axDiagnostics/reason")
            .and_then(|v| v.as_str()),
        hummingbird_context
            .pointer("/axTextSignalPresent")
            .and_then(|v| v.as_bool()),
        hummingbird_context
            .pointer("/axTextChars")
            .and_then(|v| v.as_u64()),
    ...
    Ok(json!({
      ...
      "hummingbirdContext": hummingbird_context,
      "screenContextHealth": screen_context_health,
      "lastSamplerDecision": crate::capture_sampler::last_sampler_decision_snapshot(),
      "samplerCoverage": crate::capture_sampler::sampler_decision_coverage_snapshot(50),
      "stub": false,
      "echo": payload,
    }))
}
...
pub fn app_diagnostics_report(payload: Value) -> Result<Value, String> {
    ...
    let google_cal = integration_secrets::get_credentials("google_calendar")
        .ok()
        .flatten();
...
pub fn app_delete_data_range(payload: Value) -> Result<Value, String> {
    let range = payload.get("range").and_then(|r| r.as_str()).unwrap_or("");
    let now = ts();
...
mod tests {
    ...
    fn focus(
        ...
        window_title_source: Option<&str>,
    ) -> crate::macos_frontmost::FrontmostFocus {
        crate::macos_frontmost::FrontmostFocus {
    ...
    #[test]
    fn screen_context_health_snapshot_marks_ok_when_ax_is_present() {
    ...
    #[test]
    fn screen_context_health_snapshot_marks_warn_when_ax_is_empty() {
        let health = screen_context_health_snapshot(
            Some(&focus(
                "Safari",
                Some("com.apple.Safari"),
        ...
        assert_eq!(health["windowTitleSource"], "system_events");
        assert_eq!(health["axDiagnosticReason"], "focused_element_fields_empty");
    }

    #[test]
    fn screen_context_health_snapshot_marks_warn_for_focused_tree_fallback() {
        let health = screen_context_health_snapshot(
            Some(&focus(
                "Safari",
                Some("com.apple.Safari"),
            ...
            Some(true),
            Some("ax_tree:\n- role=AXWebArea title=Inbox value="),
            Some("focused_tree"),
            Some("focused_tree_fallback"),
            Some(true),
            Some(45),
            Some(2),
        );
        assert_eq!(health["state"], "ok");
        assert_eq!(health["axSnapshotSource"], "focused_tree");
    }
    ...
    fn screen_context_health_snapshot_marks_window_tree_fallback_as_readable() {
        let health = screen_context_health_snapshot(
            ...
            Some(true),
            Some("ax_window_tree:\n- role=AXStaticText title=Inbox value=Project notes"),
        ...
        assert_eq!(health["state"], "ok");
        assert_eq!(health["axSnapshotPresent"], true);
        assert_eq!(health["axSnapshotSource"], "focused_window_tree");
    }

    #[test]
    fn screen_context_health_snapshot_marks_warn_when_ax_has_no_text_signal() {
        let health = screen_context_health_snapshot(
            Some(&focus(
                "Safari",
                Some("com.apple.Safari"),
    ...
    #[test]
    fn screen_context_health_snapshot_marks_error_when_accessibility_is_missing() {
        let health = screen_context_health_snapshot(
            Some(&focus(
                "Safari",
                Some("com.apple.Safari"),
            ...
            Some("role=AXWebArea"),
            Some("focused_element"),
            Some("accessibility_untrusted"),
            Some(false),
            Some(14),
            Some(1),
        );
        assert_eq!(health["state"], "error");
        assert_eq!(health["label"], "Screen context blocked");
    }
}
