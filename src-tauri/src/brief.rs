...
pub fn build_memory_digest(lang: &str) -> Value {
    ...
    let summaries =
        summarizer_store::get_summaries_in_window(start_ms, now_ms, lang).unwrap_or_default();
...
pub fn load_kioku_brief_signals(limit_decisions: usize, limit_kioku: usize) -> Value {
    ...
    let decisions = crate::kioku_decision_graph::fetch_decision_graph_hits(&conn, limit_decisions)
        .unwrap_or_default();
    let kioku = crate::kioku_decision_graph::fetch_recent_kioku_hits(&conn, limit_kioku)
        .unwrap_or_default();
    json!({
...
fn decision_hit_to_brief_item(hit: &DecisionGraphHit, priority: u64) -> Value {
    ...
    let why_now = if pending > 0 {
        format!("{pending} pending follow-up(s) linked in your decision graph.")
    } else {
...
pub fn merge_kioku_signals_into_brief(brief: &mut Value, signals: &Value) {
    ...
    if !kioku.is_empty() {
        let ctx: Vec<Value> = kioku.iter().take(3).map(kioku_hit_to_context_ref).collect();
        if let Some(first) = items.first_mut().and_then(|v| v.as_object_mut()) {
            ...
            for row in existing
                .into_iter()
                .take(3usize.saturating_sub(merged.len()))
            {
                merged.push(row);
            }
