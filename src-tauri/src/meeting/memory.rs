...
pub fn auto_ingest_enabled() -> bool {
  settings_store::load()
    ...
    .and_then(|doc| {
      doc
        .pointer("/sections/meetings/autoIngestToMemory")
        .and_then(|v| v.as_bool())
    })
    .unwrap_or(true)
}

fn build_snippet(meeting_id: &str, title: &str, summary_text: Option<&str>) -> Result<String, String> {
  let segs = meeting_store::list_transcript_final(meeting_id)?;
  let mut lines: Vec<String> = Vec::new();
  if let Some(sum) = summary_text {
    let t = sum.trim();
    if !t.is_empty() {
      lines.push(format!("Summary: {}", t.chars().take(600).collect::<String>()));
    }
  }
  for seg in segs.iter().take(24) {
    let sp = seg.get("speaker").and_then(|x| x.as_str()).unwrap_or("Speaker");
  ...
  if lines.is_empty() {
    lines.push(format!("Meeting \"{}\" (no transcript segments yet).", title));
  }
