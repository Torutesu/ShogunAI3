...
pub fn streaming_enabled() -> bool {
    crate::settings_store::load()
        .ok()
        .and_then(|doc| {
            doc.pointer("/sections/meetings/liveSttStreaming")
                .and_then(|v| v.as_bool())
        })
        .unwrap_or(true)
}

pub(crate) fn spawn_mic_stream(
    app: AppHandle,
    meeting_id: String,
    ...
    tauri::async_runtime::spawn(async move {
        if let Err(e) = run_pcm_stream(&app, &meeting_id, "self", stop, move || {
            mic_track
                .lock()
                .ok()
...
pub(crate) fn spawn_system_stream(
    app: AppHandle,
    meeting_id: String,
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Err(e) = run_pcm_stream(&app, &meeting_id, "other", stop, move || {
            let buf = match system_buf.lock() {
                Ok(b) => b,
