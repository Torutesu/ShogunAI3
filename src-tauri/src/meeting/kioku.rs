...
use crate::{
    extraction_jobs, meeting_store, mem_captures, memory_obs, memory_store, settings_store,
};
...
pub fn meeting_extraction_enabled() -> bool {
    settings_store::load()
        .ok()
        .and_then(|doc| {
            doc.pointer("/sections/kioku_graph/meeting_extraction_enabled")
...
mod tests {
    ...
    fn ingest_skips_when_disabled() {
        let _db = memory_store::testkit::TestDbGuard::new("meeting-kioku-ingest");
        let _settings = settings_store::TestSettingsGuard::new("meeting-kioku-ingest");
        // Default in test env may be enabled; call with empty settings path behavior.
        let meeting_id = meeting_store::new_uuid();
        meeting_store::meeting_insert(&meeting_id, 1_000, None, None, Some("Test"), None).unwrap();
