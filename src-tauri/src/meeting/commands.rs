...
mod tests {
  ...
  fn meeting_list_orders_desc_and_filters_window() {
    ...
    assert_eq!(ids, vec!["mtg-new", "mtg-mid", "mtg-old"], "started_at DESC");
  ...
  fn note_block_append_edit_delete_flow() {
    ...
    let b1 = shogun_meeting_note_append_block(json!({ "meeting_id": "mtg-n", "text": "first" }))
      .expect("append 1");
    ...
    let b2 = shogun_meeting_note_append_block(json!({ "meeting_id": "mtg-n", "text": "second" }))
      .expect("append 2");
    ...
    let contents: Vec<&str> = blocks.iter().filter_map(|b| b["content"].as_str()).collect();
    assert_eq!(contents, vec!["first", "in between", "second"]);

    // Edit keeps origin 'user' as-is.
    let edited = shogun_meeting_note_edit_block(json!({ "block_id": b1_id, "text": "first v2" }))
      .expect("edit ok");
    assert_eq!(edited["content"], json!("first v2"));
    assert_eq!(edited["origin"], json!("user"));

    // Delete.
    let deleted = shogun_meeting_note_delete_block(json!({ "block_id": b1_id })).expect("delete ok");
  ...
  fn note_edit_transitions_ai_origin_to_ai_edited() {
    ...
    let edited = shogun_meeting_note_edit_block(json!({ "block_id": "blk-ai", "text": "tweaked" }))
      .expect("edit ok");
  ...
  fn transcript_get_locks_segment_json_shape() {
    ...
    meeting_store::insert_transcript_segment("mtg-t", "seg-1", 0, 1500, "self", "hello world", Some(0.95), true)
      .expect("insert segment");
    // Non-final segments are excluded from the final transcript.
    meeting_store::insert_transcript_segment("mtg-t", "seg-2", 1500, 2000, "other_1", "partial", None, false)
  ...
  fn transcript_for_block_resolves_source_segments() {
    ...
    meeting_store::insert_transcript_segment("mtg-bs", "seg-a", 0, 1000, "self", "cited text", Some(1.0), true)
      .expect("insert segment");
    meeting_store::insert_note_block("mtg-bs", "blk-1", 0, "summary", "ai", &["seg-a".to_string()])
      .expect("insert block");
  ...
  fn link_and_resolve_client_storage_key() {
    ...
    // Link, then resolve.
    let linked = shogun_meeting_link_client_note(json!({ "meeting_id": "mtg-a", "storage_key": "key-1" }))
      .expect("link ok");
    ...
    let again = shogun_meeting_link_client_note(json!({ "meeting_id": "mtg-a", "storage_key": "key-1" }))
      .expect("relink ok");
    ...
    // the original owner.
    let conflict = shogun_meeting_link_client_note(json!({ "meeting_id": "mtg-b", "storage_key": "key-1" }))
    ...
    let err = shogun_meeting_resolve_by_storage_key(json!({})).expect_err("storage_key required");
    assert_eq!(err, "storage_key is required");
    let err = shogun_meeting_link_client_note(json!({ "meeting_id": "ghost", "storage_key": "key-2" }))
  ...
  fn meetings_search_matches_title_transcript_and_notes() {
    ...
    meeting_store::insert_transcript_segment("mtg-s2", "seg-s", 0, 1000, "self", "discussing zebras", Some(1.0), true)
      .expect("insert segment");
  ...
  fn templates_list_returns_builtins_sorted_by_name() {
    ...
    let names: Vec<&str> = templates.iter().filter_map(|t| t["name"].as_str()).collect();
    assert_eq!(
      names,
      vec![
        "1-on-1",
    ...
    assert!(t["sections"].as_array().map(|a| !a.is_empty()).unwrap_or(false));
  ...
  fn purge_cascades_to_transcript_and_notes() {
    ...
    meeting_store::insert_transcript_segment("mtg-p", "seg-p", 0, 1000, "self", "bye", None, true)
      .expect("insert segment");
    meeting_store::insert_note_block("mtg-p", "blk-p", 0, "note", "user", &[]).expect("insert block");
    ...
    let transcript = shogun_meeting_transcript_get(json!({ "meeting_id": "mtg-p" })).expect("ok");
    assert_eq!(transcript["segments"].as_array().expect("segments").len(), 0);
