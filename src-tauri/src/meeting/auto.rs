...
pub fn auto_start_on_video_detect_enabled() -> bool {
    settings_store::load()
        .ok()
        .and_then(|doc| {
            doc.pointer("/sections/meetings/autoStartOnVideoDetect")
...
pub fn auto_start_mic_enabled() -> bool {
    settings_store::load()
        .ok()
        .and_then(|doc| {
            doc.pointer("/sections/meetings/autoStartMicOnVideoDetect")
                .and_then(|v| v.as_bool())
        })
        .unwrap_or(true)
}
...
pub fn try_start_from_video_detect(
    ...
    if meeting_store::meeting_insert(&id, started, None, Some(provider), Some(title.trim()), None)
        .is_err()
    {
        return None;
    }
