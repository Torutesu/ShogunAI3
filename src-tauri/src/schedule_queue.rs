...
pub fn append(payload: &Value) -> Result<Value, String> {
    ...
    crate::app_events::emit_action_layer_refresh("queue.tasks.append", Some(result.clone()));
    Ok(result)
}
...
pub fn remove(payload: &Value) -> Result<Value, String> {
    ...
    crate::app_events::emit_action_layer_refresh("queue.tasks.remove", Some(result.clone()));
    Ok(result)
}
...
pub fn retry(payload: &Value) -> Result<Value, String> {
    ...
    crate::app_events::emit_action_layer_refresh("queue.tasks.retry", Some(result.clone()));
    Ok(result)
}
...
mod tests {
    ...
    fn append_list_and_remove_round_trip() {
        ...
        path.push(format!(
            "shogun-schedule-queue-test-{}.json",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&path);

        let saved = append_at_path(&path, &json!({ "title": "Task A" })).expect("append");
    ...
    fn retry_requeues_existing_payload() {
        let mut path = std::env::temp_dir();
        path.push(format!(
            "shogun-schedule-queue-retry-test-{}.json",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&path);

        let saved = append_at_path(&path, &json!({ "title": "Task B" })).expect("append");
        let id = saved["id"].as_str().expect("id").to_string();
