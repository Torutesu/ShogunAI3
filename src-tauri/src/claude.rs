...
fn notes_from_settings() -> Vec<(String, String)> {
  ...
  doc
    .pointer("/sections/claude/notes")
    .and_then(|v| v.as_array())
    .map(|arr| {
      arr.iter()
        .filter_map(|row| {
          let title = row.get("title").and_then(|v| v.as_str()).unwrap_or("Claude note");
...
async fn validate_key(key: &str) -> Result<(), String> {
  ...
  let text: String = resp.text().await.unwrap_or_default().chars().take(400).collect();
