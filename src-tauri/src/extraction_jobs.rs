...
pub fn enqueue(
    ...
    if job_kind == JOB_KIND_EXTRACT && capture_id.is_none() {
        return Err(
            "extraction_jobs::enqueue: capture_id is required for extract jobs".to_string(),
        );
    }
    conn.execute(
...
mod tests {
    ...
    fn enqueue_extract_requires_capture_id() {
        let conn = open_test_conn();
        let err = enqueue(None, JOB_KIND_EXTRACT, 2_000, None, &conn)
            .expect_err("None rejected for extract");
    ...
    fn enqueue_invalid_capture_id_returns_fk_error() {
        ...
        let err =
            enqueue(Some(9_999), JOB_KIND_EXTRACT, 2_000, None, &conn).expect_err("FK violation");
