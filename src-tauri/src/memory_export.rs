...
pub fn validate_header(line: &str) -> Result<ExportHeader, String> {
    let h: ExportHeader =
        serde_json::from_str(line.trim()).map_err(|e| format!("header parse: {}", e))?;
    if h.format != FORMAT {
...
pub fn export_to_writer<W: Write>(conn: &rusqlite::Connection, out: &mut W) -> Result<u64, String> {
    ...
    let header_line =
        serde_json::to_string(&header).map_err(|e| format!("header serialize: {}", e))?;
...
pub fn import_from_reader<R: BufRead>(
    ...
    for line_result in lines {
        ...
        let v: Value = serde_json::from_str(trimmed).map_err(|e| format!("row parse: {}", e))?;
...
mod tests {
    ...
    fn count_rows(conn: &Connection) -> i64 {
        conn.query_row("SELECT COUNT(*) FROM mem_items", [], |r| r.get(0))
            .unwrap_or(0)
    }
    ...
    fn t2_round_trip_preserves_data() {
        ...
        let n =
            import_from_reader(&conn2, std::io::BufReader::new(buf.as_slice())).expect("import");
        assert_eq!(n, 2);
    ...
    fn t3_import_replace_semantics() {
        ...
        let n = import_from_reader(&conn, std::io::BufReader::new(buf.as_slice())).expect("import");
        assert_eq!(n, 5);
    ...
    fn t6_malformed_row_rolls_back() {
        ...
        let dup_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM mem_items WHERE id = 'dup'", [], |r| {
                r.get(0)
            })
            .expect("count");
    ...
    fn t9_header_line_first() {
        ...
        assert_eq!(row.get("id").and_then(|v| v.as_str()), Some("t9-row"));
    }
