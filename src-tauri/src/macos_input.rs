...
static LAST_FOCUS_KEY: Mutex<Option<String>> = Mutex::new(None);
...
fn maybe_wake_click(now: u64) {
    ...
    if prev > 0
        && !should_wake_after_debounce(
            now,
            LAST_CLICK_WAKE_MS.load(Ordering::Relaxed),
            CLICK_DEBOUNCE_MS,
        )
    {
        return;
...
mod imp {
    ...
    fn focus_key(app: &str, bundle_id: Option<&str>, window_title: Option<&str>) -> String {
        let bundle = bundle_id.unwrap_or("").trim();
        let title = window_title.unwrap_or("").trim();
        format!("{app}\u{1f}{bundle}\u{1f}{title}")
    }
    ...
    fn maybe_focus_change(app: &str, bundle_id: Option<&str>, window_title: Option<&str>) {
        let title = window_title
            .map(str::trim)
            .filter(|s| !s.is_empty() && *s != app);
        let focus_key = focus_key(app, bundle_id, title);
        let changed = if let Ok(mut last) = LAST_FOCUS_KEY.lock() {
            if last.as_deref() == Some(focus_key.as_str()) {
                false
            } else {
                *last = Some(focus_key);
                true
        ...
        if crate::capture_sampler::app_excluded(&filters, app, bundle_id) {
            return;
        }
        request_sampler_wake();
        let detail = match title {
            Some(t) => format!("focus → {app} — {}", clip_chars(t, 64)),
            None => format!("focus → {app}"),
        };
        crate::capture_events::record_live(app, "focus", &detail);
        let entity = bundle_id
            .map(|id| format!("focus:{}", id.to_ascii_lowercase()))
            .unwrap_or_else(|| format!("focus:{}", app.to_ascii_lowercase()));
    ...
    extern "C" fn tap_callback(
        ...
        let app = frontmost
            ...
            .to_string();
        let filters = crate::capture_sampler::load_privacy_filters();
        if crate::capture_sampler::app_excluded(
            &filters,
            &app,
            frontmost
                .as_ref()
                .and_then(|focus| focus.bundle_id.as_deref()),
        ) {
            return event;
        }
    ...
    pub fn start_event_tap_thread() {
        ...
        std::thread::spawn(|| {
            ...
            if tap.is_null() {
                log::warn!(
                    "macos_input: CGEventTapCreate failed — grant Accessibility + Input Monitoring"
                );
                return;
            }
