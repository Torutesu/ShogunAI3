...
fn count_query(conn: &Connection, sql: &str, params: &[&dyn rusqlite::ToSql]) -> Result<i64, String> {
  conn
...
pub fn run_dry_run(conn: &Connection, now_ms: i64) -> Result<Stage5DryRunReport, String> {
  ...
  Ok(Stage5DryRunReport {
    ...
    physical_delete,
    storage,
    legacy_sources: CAPTURE_LEGACY_SOURCES.iter().map(|s| (*s).to_string()).collect(),
...
pub fn physical_delete_old_capture_rows(
  conn: &Connection,
  now_ms: i64,
) -> Result<usize, String> {
  let cutoff = now_ms - STAGE5_SOFT_RETIRE_GRACE_DAYS * MS_PER_DAY;
...
mod tests {
  ...
  fn seed_mem_item(
    ...
    let blob: Option<Vec<u8>> = embedding.map(|v| v.iter().flat_map(|f| f.to_le_bytes()).collect());
    conn
      .execute(
  ...
  fn dry_run_soft_retire_counts_embedding_blobs() {
    let conn = open_test_conn();
    seed_mem_item(&conn, "m_a", "capture_sampler", None, Some(&[0.1, 0.2]), 1_000);
    seed_mem_item(&conn, "m_b", "capture_ax", None, None, 2_000);
    let report = run_dry_run(&conn, 5_000).expect("dry run");
  ...
  fn dry_run_ttl_block_counts_only_done_with_raw() {
    let conn = open_test_conn();
    let now = 100_000;
    seed_capture(&conn, 1_000, now - 1, "done", Some("rawtext"), Some("/tmp/a"));
    ...
    seed_capture(&conn, 4_000, now - 1, "done", Some("rawtext"), Some("/tmp/b"));
  ...
  fn dry_run_physical_delete_only_picks_rows_past_grace_window() {
    ...
    seed_mem_item(&conn, "m_recent", "capture_sampler", Some(now - 1), None, 1_000);
  ...
  fn soft_retire_marks_only_active_legacy_rows() {
    ...
    let valid_to_a: Option<i64> = conn
      .query_row("SELECT valid_to FROM mem_items WHERE id = 'm_a'", [], |r| r.get(0))
      .unwrap();
    assert_eq!(valid_to_a, Some(9_999));
    let valid_to_d: Option<i64> = conn
      .query_row("SELECT valid_to FROM mem_items WHERE id = 'm_d'", [], |r| r.get(0))
      .unwrap();
  ...
  fn physical_delete_drops_only_aged_legacy_rows() {
    ...
    let remaining: Vec<String> = {
      let mut stmt = conn.prepare("SELECT id FROM mem_items ORDER BY id").unwrap();
      stmt
        .query_map([], |r| r.get::<_, String>(0))
        .unwrap()
        .filter_map(|x| x.ok())
