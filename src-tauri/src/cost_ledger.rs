...
pub fn sum_cost_in_window(conn: &Connection, since_ms: i64, until_ms: i64) -> Result<f64, String> {
    let v: f64 = conn
        .query_row(
...
mod tests {
    ...
    fn calc_cost_with_cache_realistic_kioku_extraction_breakdown() {
        ...
        assert!(
            cost < 0.005,
            "per-job under cached path should drop below $0.005"
        );
    }
    ...
    fn record_inserts_one_row_with_payload() {
        ...
        let id =
            record(&entry(1_000, PURPOSE_EXTRACTION, 2_000, 400, 0.004), &conn).expect("record ok");
        assert!(id > 0);
        let count: i64 = conn
    ...
    fn cap_status_pauses_extraction_when_at_or_over_cap() {
        ...
        match s {
            CapStatus::Pause { reason } => {
                assert!(reason.contains("pausing extraction"), "got: {}", reason)
            }
