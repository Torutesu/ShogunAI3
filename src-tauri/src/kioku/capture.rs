...
fn ax_diff_lines(prev: &[String], next: &[String]) -> usize {
    ...
    let next_set: std::collections::HashSet<&str> = next.iter().map(|s| s.as_str()).collect();
...
pub fn should_capture(sig: &CaptureSignal, ring: &SigRing, cfg: &FilterConfig) -> CaptureDecision {
...
mod tests {
    ...
    fn route_capture_records_capture_and_enqueues_extract_job() {
        ...
        let cap_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM mem_captures WHERE id = ?1",
      ...
      .expect("count");
        assert_eq!(
            job_count, 1,
            "expected exactly one extract job for the capture"
        );
    }
    ...
    fn window_within_window_aggregates_in_bucket() {
        let mut w = BatchedDedupWindow::new(30_000);
        assert_eq!(
            w.observe(pc("k", 1_000, 5_000, 0x1)),
            WindowOutcome::Pending
        );
        // 10 s later, same key, still inside window: pending.
        assert_eq!(
            w.observe(pc("k", 11_000, 5_000, 0x2)),
            WindowOutcome::Pending
        );
    ...
    fn sigring_remembers_ax_dump_per_key() {
        ...
        r.put_ax(
            key,
            vec!["role=AXTextField".to_string(), "value=hi".to_string()],
        );
    ...
    fn sigring_ax_dump_keys_are_independent() {
        ...
        assert_eq!(
            r.last_ax_for("a|win1").unwrap().lines,
            vec!["x".to_string()]
        );
        assert_eq!(
            r.last_ax_for("b|win2").unwrap().lines,
            vec!["y".to_string()]
        );
