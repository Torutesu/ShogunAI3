...
fn backfill_phase2_columns(conn: &Connection) -> Result<(), String> {
    ...
    let mut stmt = conn
        .prepare("SELECT id, source FROM mem_items WHERE node_kind IS NULL")
        .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn ensure_creates_mem_captures_table_with_required_shape() {
        ...
        ensure_kioku_graph_schema(&conn).expect("ensure ok");
        assert!(
            table_exists(&conn, "mem_captures"),
            "mem_captures should exist"
        );
    ...
    fn ensure_new_tables_accept_minimal_insert() {
      ...
      .expect("insert mem_captures");
        ...
        // extraction_jobs
        conn.execute("INSERT INTO extraction_jobs (created_at) VALUES (100)", [])
    ...
    fn backfill_populates_node_kind_from_source() {
        ...
        assert_eq!(
            read_phase2_fields(&conn, "m_cap").0.as_deref(),
            Some("capture_summary")
        );
        assert_eq!(
            read_phase2_fields(&conn, "m_ax").0.as_deref(),
            Some("capture_summary")
        );
        assert_eq!(
            read_phase2_fields(&conn, "m_cal").0.as_deref(),
            Some("event")
        );
        assert_eq!(
            read_phase2_fields(&conn, "m_mail").0.as_deref(),
            Some("note")
        );
        assert_eq!(
            read_phase2_fields(&conn, "m_meet").0.as_deref(),
            Some("event")
        );
        assert_eq!(
            read_phase2_fields(&conn, "m_other").0.as_deref(),
            Some("note")
        );
    }
    ...
    fn derive_node_kind_capture_sources_become_capture_summary() {
        assert_eq!(
            derive_node_kind(Some("screen"), "capture_sampler"),
            "capture_summary"
        );
        assert_eq!(
            derive_node_kind(Some("screen"), "capture_ax"),
            "capture_summary"
        );
    }
    ...
    fn derive_node_kind_calendar_becomes_event() {
        assert_eq!(
            derive_node_kind(Some("connector"), "google_calendar"),
            "event"
        );
    }
    ...
    fn derive_node_kind_meeting_family_becomes_event() {
        ...
        assert_eq!(
            derive_node_kind(Some("meeting"), "meetings_granola"),
            "event"
        );
    ...
    fn derive_node_kind_unknown_source_falls_back_to_note() {
        ...
        assert_eq!(
            derive_node_kind(Some("user"), "telemetry_chat_context"),
            "note"
        );
