...
pub fn query_cost_stats(
    ...
    let spent =
        crate::cost_ledger::sum_cost_in_window(conn, month_start, now_ms.saturating_add(1))?;
...
pub fn query_graph_stats(conn: &Connection) -> Result<GraphStats, String> {
    ...
    s.mem_items_active = conn
        .query_row(
            "SELECT COUNT(*) FROM mem_items WHERE valid_to IS NULL",
            [],
            |r| r.get(0),
        )
        .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn seed_node(conn: &Connection, id: &str, node_kind: Option<&str>, valid_to: Option<i64>) {
        conn.execute(
    ...
    fn cost_stats_sums_only_current_month() {
        ...
        let april = Utc
            ...
            .timestamp_millis();
        let march = Utc
            .with_ymd_and_hms(2026, 3, 31, 23, 59, 0)
            .single()
            .unwrap()
            .timestamp_millis();
        ...
        .unwrap();
        let now = Utc
            .with_ymd_and_hms(2026, 4, 26, 17, 0, 0)
            .single()
            .unwrap()
            .timestamp_millis();
    ...
    fn graph_stats_groups_by_edge_type_active_only() {
        ...
        let mentions = s
            .by_edge_type
            .iter()
            .find(|x| x.edge_type == "mentions")
            .map(|x| x.count);
        let follows = s
            .by_edge_type
            .iter()
            .find(|x| x.edge_type == "follows_up")
            .map(|x| x.count);
