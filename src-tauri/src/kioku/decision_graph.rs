...
pub fn fetch_recent_kioku_hits(conn: &Connection, limit: usize) -> Result<Vec<KiokuHit>, String> {
    let sql = "
  ...
  ";
    ...
    for row in rows {
        ...
        let relevance_score = decay.map(|d| d.clamp(0.0, 1.0)).unwrap_or(0.5);
        out.push(KiokuHit {
            doc_id: id,
            title,
            snippet: snippet_opt,
...
mod tests {
    ...
    fn kioku_hit_serializes_to_amc_schema_shape() {
        ...
        assert_eq!(
            v["title"],
            serde_json::json!("Idea: snippet clip default 500")
        );
        assert_eq!(
            v["snippet"],
            serde_json::json!("Pack files leak AX dumps...")
        );
    ...
    fn fetch_decision_graph_hits_returns_decisions_with_summary_from_snippet() {
        ...
        seed_node(
            &conn,
            "m_dec_1",
            "Decision: ship Stage 1",
            "Stage 1 schema additions land in T1.",
            "decision",
            None,
            1_000,
        );
    ...
    fn fetch_decision_graph_hits_counts_pending_follow_ups() {
        let conn = open_test_conn();
        seed_node(
            &conn, "m_dec_1", "Decision", "Plan Q2", "decision", None, 1_000,
        );
