...
pub fn embed_claim_blocking(claim: &str) -> Option<Vec<f32>> {
    #[cfg(test)]
    {
        let run_live_embeddings = std::env::var("SHOGUN_RUN_EXTRACTION_EMBEDDING_TESTS")
            .map(|v| matches!(v.as_str(), "1" | "true" | "TRUE"))
            .unwrap_or(false);
        if !run_live_embeddings {
            return None;
        }
    }
    ...
    let result =
        tauri::async_runtime::block_on(async move { crate::embeddings::embed_one(&clipped).await });
    match result {
...
pub fn run_worker_tick<C: ExtractionClient>(
    ...
    let status =
        crate::cost_ledger::evaluate_cap_status(spent, monthly_cap_usd, cap_action, fallback_model);
...
pub fn process_one_job<C: ExtractionClient>(
    ...
    match client.extract(&ctx) {
        Ok(resp) => {
            ...
            .unwrap_or(0.0);
            let meta_json =
                if resp.cache_creation_input_tokens > 0 || resp.cache_read_input_tokens > 0 {
                    Some(format!(
            ...
            match err {
                ExtractionError::Transient(msg) => {
                 ...
                 WHERE id = ?3",
                        ...
                        .map_err(|e| {
                            format!("process_one_job mark failed (retries exhausted): {}", e)
                        })?;
...
mod tests {
    ...
    fn is_same_fact_stage3_blocked_by_low_levenshtein_ratio() {
        ...
        let b = fr(
            ...
            "Alex",
            "works_at",
            "is the CTO of Beta Industries Tokyo",
        );
        assert!(!is_same_fact(&a, &b, None, None));
    }
    ...
    impl FakeClient {
        ...
        fn err_transient(msg: &str) -> Self {
            FakeClient {
                response: std::cell::RefCell::new(Some(Err(ExtractionError::Transient(
                    msg.into(),
                )))),
            }
        }
        fn err_permanent(msg: &str) -> Self {
            FakeClient {
                response: std::cell::RefCell::new(Some(Err(ExtractionError::Permanent(
                    msg.into(),
                )))),
            }
        }
    ...
    fn resolve_write_does_not_merge_when_embeddings_orthogonal() {
        ...
        match outcome {
            ...
            other => panic!(
                "expected Superseded for orthogonal embeddings, got {:?}",
                other
            ),
        }
    }
    ...
    fn resolve_write_adds_relation_edges_for_new_node() {
        ...
        let target_b = resolve_write(
            &fact("Q2 launch", "is_event", "Q2 launch event"),
            None,
            None,
            1_000,
            &conn,
