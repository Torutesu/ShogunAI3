...
pub fn traverse_subgraph(
  ...
  out.sort_by(|a, b| {
    a.depth
      .cmp(&b.depth)
      .then_with(|| b.path_score.partial_cmp(&a.path_score).unwrap_or(std::cmp::Ordering::Equal))
  });
  Ok(out)
}
...
pub fn rank_subgraph_hits(
  ...
  let mut out: Vec<RankedHit> = nodes
    .iter()
    .map(|n| {
      let decay = decay_lookup.get(&n.id).copied().unwrap_or(0.0).max(RANKER_FLOOR);
...
pub fn bump_access_for_hits(
  ...
  for id in hit_ids {
    ...
    let access_term = crate::decay::access_boost(new_access, crate::decay::DECAY_ACCESS_COUNT_CAP);
...
mod tests {
  ...
  fn pick_entry_excludes_retired_nodes() {
    let conn = open_test_conn();
    seed_node(&conn, "m_old", Some(&[1.0, 0.0]), 0.6, Some(2_000), "entity");
  ...
  fn pick_entry_truncates_to_limit() {
    ...
    for i in 0..5 {
      ...
      seed_node(&conn, &format!("m_{i}"), Some(&[val, 0.0]), 0.6, None, "entity");
    }
    let q = vec![1.0_f32, 0.0];
    let r = pick_entry_nodes(&conn, &q, 3).unwrap();
    assert_eq!(r.len(), 3);
  ...
  fn traverse_walks_to_max_depth_and_no_further() {
    ...
    assert!(!ids.contains(&"m_d".to_string()), "depth-3 node leaked into depth-2 walk");
  }
  ...
  fn traverse_skips_retired_edges_and_nodes() {
    ...
    assert!(!ids.contains(&"m_dead_node".to_string()), "retired node leaked");
  }
  ...
  fn ranker_orders_by_combined_score() {
    let nodes = vec![
      TraversalNode { id: "lo".into(), depth: 0, path_score: 1.0 },
      TraversalNode { id: "hi".into(), depth: 1, path_score: 0.6 },
    ];
  ...
  fn ranker_floor_protects_unseeded_similarity() {
    let nodes = vec![
      TraversalNode { id: "ok".into(), depth: 1, path_score: 1.0 },
    ];
