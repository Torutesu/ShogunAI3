...
pub fn record_proposal(conn: &Connection, edge_type: &str, now_ms: i64) -> Result<(), String> {
...
mod tests {
    ...
    fn record_proposal_increments_seen_count_and_updates_last_seen() {
      ...
      .unwrap();
        ...
        assert_eq!(
            row.1, 1_000,
            "first_seen_at must not change after the initial insert"
        );
    ...
    fn record_proposal_does_not_overwrite_human_review_decision() {
      ...
      .unwrap();
        assert_eq!(
            row.0, REVIEW_REJECTED,
            "human decision must persist across re-sightings"
        );
    ...
    fn record_proposal_trims_whitespace() {
        ...
        let edge_type: String = conn
            .query_row(
                "SELECT edge_type FROM edge_type_proposals LIMIT 1",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn set_review_accepts_with_optional_note() {
        let conn = open_test_conn();
        record_proposal(&conn, "discusses", 1_000).unwrap();
        let n = set_review_status(
            &conn,
            "discusses",
            REVIEW_ACCEPTED,
            Some("matches AMC schema"),
        )
        .unwrap();
        assert_eq!(n, 1);
        let row: (i64, Option<String>) = conn
