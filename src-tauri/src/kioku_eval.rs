...
mod tests {
  ...
  fn fixture_dir_resolves_under_repo_root() {
    let p = fixture_dir();
    assert!(p.ends_with("tests/fixtures/kioku_eval"), "got {}", p.display());
  }
  ...
  fn fixtures_load_with_expected_counts() {
    ...
    let nodes: Vec<NodeFixture> = load_jsonl(&base.join("nodes.jsonl"))
      .expect("nodes.jsonl loads");
    assert!(
      nodes.len() >= 50,
    ...
    let captures: Vec<CaptureFixture> = load_jsonl(&base.join("captures.jsonl"))
      .expect("captures.jsonl loads");
    assert!(
      captures.len() >= 20,
    ...
    assert!(same_fact.iter().any(|c| c.expected), "no expected:true cases");
    assert!(
      same_fact.iter().any(|c| !c.expected),
      "no expected:false cases"
    );

    let same_ent: Vec<SameEntityDifferentFactCase> = load_jsonl(
      &base.join("same_entity_different_fact_cases.jsonl"),
    )
...
"#;
  ...
  fn parse_jsonl_reports_lineno_on_parse_error() {
    ...
    assert!(err.contains("line 3"), "error did not mention line 3: {err}");
  }
...
"#;
  ...
  fn ndcg_at_k_penalizes_late_hits() {
    ...
    assert!(s_early > s_late, "early {} should beat late {}", s_early, s_late);
