...
pub const RETRIEVAL_ALLOWED_NODE_KINDS: &[&str] = &["entity", "event", "decision", "task", "note"];
...
pub async fn assemble_memory_hits(params: AssembleParams<'_>) -> Result<Vec<Hit>, String> {
...
pub fn filter_assembled_hits_by_kinds(hits: Vec<Hit>, want: &[String]) -> Result<Vec<Hit>, String> {
    if want.is_empty() {
        return Ok(hits);
    }
...
fn recent_active_nodes(conn: &rusqlite::Connection, limit: usize) -> Result<Vec<String>, String> {
    let mut stmt = conn
        .prepare(
...
mod tests {
    ...
    fn current_read_path_defaults_to_graph_without_settings() {
        let _settings = crate::settings_store::TestSettingsGuard::new("current-read-path-default");
...
mod real_db_smoke {
    ...
    async fn timeline_graph_real_db_smoke() {
        ...
        assert_eq!(
            graph.get("scope").and_then(|v| v.as_str()),
            Some("timeline")
        );
        assert_eq!(
        ...
        let legacy_hits = legacy
            ...
            .and_then(|v| v.as_array())
            .expect("legacy hits");
        assert!(!legacy_hits.is_empty(), "legacy timeline returned no hits");
