...
pub fn heuristic_fallback(item: &Value, source_type: &str, lang: &str) -> Summary {
    ...
    Summary {
        ...
        key_points: vec![if first_sentence.is_empty() {
            no_content
        } else {
            first_sentence
        }],
...
pub async fn summarize_item(item: &Value, lang: &str) -> Result<Summary, String> {
    ...
    match crate::llm::anthropic_tool_complete(&system, &user_content, &tool, SUMMARIZER_MODEL).await
    {
        Ok(tool_input) => match build_summary_from_tool_input(item, source_type, &tool_input, lang)
        {
            ...
            Err(e) => {
                log::warn!(
                    "summarizer tool_input parse error for {}: {}",
                    target_id_of(item),
                    e
                );
...
fn build_summary_from_tool_input(
    item: &Value,
    source_type: &str,
    input: &Value,
    lang: &str,
    ...
    match crate::llm::anthropic_tool_complete(&system, &user_content, &tool, SUMMARIZER_MODEL).await
    {
        Ok(tool_input) => {
            ...
            let key_points: Vec<String> = tool_input
                ...
                .unwrap_or_default();
            if key_points.is_empty() {
                return Ok(entity_heuristic_fallback(
                    entity_id,
                    entity_label,
                    &items,
                    lang,
...
fn entity_heuristic_fallback(
    ...
    let summary_line = match lang {
        "jp" => format!("{} 件 (要対応 {} / 注目 {})", total, high_count, med_count),
        _ => format!(
...
pub async fn summarize_year_rollup(year_start_ms: i64, lang: &str) -> Result<Summary, String> {
    ...
    if monthly.is_empty() || monthly.iter().all(is_empty_rollup) {
        return Ok(Summary {
            ...
            reason: Some(loc(
                lang,
                "No indexed activity in this window",
                "インデックス済みの活動なし",
            )),
            model: "heuristic".into(),
            schema_version: SCHEMA_VERSION,
            generated_at: crate::memory_store::now_ms() as i64,
            raw_json: json!({"rollup": "empty", "kind": RollupKind::Year.target_kind()})
                .to_string(),
            lang: lang.to_string(),
            user_priority: None,
            acknowledged_at: None,
        ...
        RollupKind::Year => (
          ...
          format!("{indexed} 件の記憶がインデックス済み — Memory → River で要約を更新できます"),
            ...
            return Ok(Summary {
                ...
                generated_at: crate::memory_store::now_ms() as i64,
                raw_json:
                    json!({"rollup": "pending", "indexed": indexed, "kind": kind.target_kind()})
                        .to_string(),
                lang: lang.to_string(),
                user_priority: None,
                acknowledged_at: None,
...
fn build_rollup_from_tool_input(
    id: &str,
    kind: RollupKind,
    input: &Value,
    lang: &str,
...
mod tests {
    ...
    fn gmail_japanese_unsubscribe_is_low() {
        ...
        let guess =
            heuristic_priority_guess(&item, "en").expect("Japanese unsubscribe should match");
        assert_eq!(guess.priority, "low");
    }
    ...
    fn gmail_body_mention_of_noreply_is_not_low() {
        ...
        assert!(
            heuristic_priority_guess(&item, "en").is_none(),
            "body mention should not trigger low"
        );
    }
