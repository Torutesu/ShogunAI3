...
fn run_osascript(script: &str) -> Result<String, String> {
    ...
    if !out.status.success() {
        ...
        let clip: String = format!("{stderr}{stdout}").chars().take(400).collect();
        return Err(format!(
...
pub fn probe_calendar() -> Result<(), String> {
    let _ = run_osascript(r#"tell application "Calendar" to get (count of calendars)"#)?;
    Ok(())
}
...
pub fn probe_reminders() -> Result<(), String> {
    let _ = run_osascript(r#"tell application "Reminders" to get (count of lists)"#)?;
    Ok(())
}
