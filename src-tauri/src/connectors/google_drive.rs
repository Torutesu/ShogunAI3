...
async fn export_file_text(token: &str, file_id: &str, mime_type: &str) -> String {
    ...
    let endpoint = format!("files/{}/export", urlencoding::encode(file_id),);
...
fn ingest_drive_file(file: &Value, body_text: &str) -> Result<(), String> {
    ...
    let short_kind = if mime.starts_with("application/vnd.google-apps.") {
        mime.trim_start_matches("application/vnd.google-apps.")
            .to_string()
    } else {
        mime.to_string()
    };
...
pub async fn sync_drive_to_memory(days: Option<u32>, max_files: usize) -> Result<Value, String> {
    ...
    let q_expr = format!("trashed = false and modifiedTime > '{}'", cutoff_iso,);
    let fields =
