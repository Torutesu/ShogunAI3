...
mod tests {
  ...
  fn tmp_path(name: &str) -> PathBuf {
    ...
    p.push(format!("kioku-backup-test-{}-{}.db", std::process::id(), name));
    p
  }
  ...
  fn default_dest_appends_iso_timestamp_in_same_dir() {
    ...
    assert!(name.contains("2025-04-26-"), "expected 2025-04-26 in {}", name);
  }
  ...
  fn backup_db_produces_a_readable_copy() {
    ...
    let title: String = copy
      .query_row("SELECT title FROM mem_items WHERE id = 'm_1'", [], |row| row.get(0))
      .expect("row");
