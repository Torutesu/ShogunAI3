...
fn parse_payment(priv_sec: Option<&Value>) -> PaymentRules {
    ...
    let enabled = ps.get("enabled").and_then(|v| v.as_bool()).unwrap_or(true);
...
fn parse_incognito(priv_sec: Option<&Value>) -> IncognitoRules {
    ...
    let enabled = inc.get("enabled").and_then(|v| v.as_bool()).unwrap_or(true);
...
pub fn is_incognito_window(rules: &IncognitoRules, app_name: &str, window_title: &str) -> bool {
    ...
    match app.as_str() {
        "safari" | "safari technology preview" => {
            rules.safari
                && (window_title.starts_with("Private — ")
        ...
        "microsoft edge" => {
            rules.edge
                && (window_title.contains("[InPrivate]") || title_lower.contains("inprivate"))
        }
        _ => false,
    }
}
