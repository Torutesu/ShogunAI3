 ...
 * Brand assets: `/assets/integrations/official/` — vendor marks where permitted.
...
function normalizeProvider(raw: any) {
  return String(raw || "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "_");
...
function readConnectedMap() {
  try {
    ...
    const raw = window.localStorage.getItem(LS_CONNECTED);
    if (!raw) return {};
...
function writeConnectedMap(map: any) {
  try {
    if (!window.localStorage) return;
    window.localStorage.setItem(LS_CONNECTED, JSON.stringify(map));
  } catch (_) {
    /* ignore */
  }
}

function setConnected(slug: any, on: any) {
...
function getConnected(slug: any) {
  const m = readConnectedMap();
...
function dispatchCredEvent() {
  try {
    window.dispatchEvent(new Event("shogun-credentials-updated"));
  } catch (_) {
    /* ignore */
  }
}
...
function readGcalMock() {
  try {
    ...
    const raw = window.localStorage.getItem(LS_GCAL);
...
function readGmailMock() {
  try {
    if (!window.localStorage) return { configured: false, tokenRefreshReady: false };
    const raw = window.localStorage.getItem(LS_GMAIL);
    if (!raw) return { configured: false, tokenRefreshReady: false };
    const o = JSON.parse(raw);
    if (!o || typeof o !== "object") return { configured: false, tokenRefreshReady: false };
    return {
...
function writeGcalMock(patch: any) {
  try {
    if (!window.localStorage) return;
    const prev = (() => {
      try {
    ...
    const next = { ...prev, ...patch };
    window.localStorage.setItem(LS_GCAL, JSON.stringify(next));
    dispatchCredEvent();
  } catch (_) {
    /* ignore */
  }
...
function hydrateTools(tools: any) {
  const m = readConnectedMap();
  return tools.map((t: any) => ({ ...t, connected: !!m[t.slug] }));
}

function getIconFile(slug: any) {
  return ICON_BY_SLUG[normalizeProvider(slug)] || null;
}

const notImpl = (message: any, echo: any) => ({
  notImplemented: true,
  message,
  stub: false,
  echo: echo || {},
 ...
 */
function mockIntegrationPayload(command: any, echo: any) {
  const e = echo || {};
  switch (command) {
...
export const ShogunIntegrationConnectors = {
  ICON_BY_SLUG,
  REGISTERED,
  normalizeProvider,
  getIconFile,
