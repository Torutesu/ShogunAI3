...
var storageKeyRef: any = null;
var titleRef = 'Untitled';
var startedAtRef = 0;

function fmtElapsedMs(ms: any) {
...
async function start(opts: any) {
  ...
  if (!window.navigator.mediaDevices || typeof (window as any).MediaRecorder === 'undefined') {
  ...
  try {
    ...
    for (var i = 0; i < candidates.length; i++) {
      if ((window as any).MediaRecorder.isTypeSupported(candidates[i])) {
        mime = candidates[i] as string;
        break;
      }
    }
    var mr = mime ? new (window as any).MediaRecorder(stream, { mimeType: mime }) : new (window as any).MediaRecorder(stream);
    mediaRecorder = mr;

    mr.ondataavailable = function (e: any) {
