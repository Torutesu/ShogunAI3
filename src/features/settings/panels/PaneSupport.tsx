    ...
    <Pane title="Support" jp="支援">
      <div className="s-field-hint" style={{ marginBottom: 14, padding: 12, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', fontSize: 12, lineHeight: 1.55 }}>
        ...
        {PRODUCT.supportMailto ? (
          ...
          <span>Support email is not configured yet.</span>
        )}
        {PRODUCT.supportMailto ? (
          <>
            （<code style={{ fontSize: 10 }}>PRODUCT.supportMailto</code> を販売用アドレスに差し替えてください）。
          </>
        ) : (
          <>
            （<code style={{ fontSize: 10 }}>PRODUCT.supportMailto</code> を販売用アドレスで設定してください）。
          </>
        )}
        Discord 等は準備中の場合があります。
      ...
      <div className="s-card">
        <Row title="Email support" desc="Bug reports, licensing, and setup — use the address configured for your product build.">
          {PRODUCT.supportMailto ? (
            ...
            <span className="s-field-hint">No support email configured.</span>
          )}
        </Row>
        ...
        <Row
          ...
          desc="Desktop: Tauri updater (see tauri.conf.json — endpoints + pubkey). Browser mock: no update."
        >
