...
export function ScreenIntegrations() {
  ...
  const refreshDriveStatus = React.useCallback(() => {
    return runRuntimeAction('integrations.credentials_status', { provider:'google_drive' }, { silentError:true }).then((res) => {
      if (res.ok && res.data) {
...
if (typeof window !== 'undefined') {
  (window as unknown as Record<string, unknown>).ScreenWork = ScreenWork;
  (window as unknown as Record<string, unknown>).ScreenCapture = ScreenCapture;
  (window as unknown as Record<string, unknown>).ScreenIntegrations = ScreenIntegrations;
  (window as unknown as Record<string, unknown>).ScreenSettings = ScreenSettings;
}
